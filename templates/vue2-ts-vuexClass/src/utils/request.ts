...
class HttpRequest {
  ...
  interceptors(instance: any, url?: string) {
    ...
    instance.interceptors.response.use((res: AxiosResponse) => {
      ...
      if (status === 200 && data && data.result === 'success') { return data } // 请求成功
...
const conbineOptions = (_opts: any, data: Datas, method: Methods): AxiosRequestConfig => {
  ...
  const urlObj = JSON.parse(getLocalStorage('urlObj') || '')
  const _data = { ...conmomPrams, ...opts.data, ...data, ...urlObj }
  const options = {
    method: opts.method || data.method || method || 'GET',
    url: opts.url,
    withCredentials: true,
    mode: 'no-cors',
    // header: { 'user-token': 'xxx' },
    baseURL
  }
